...
from onpolicy.envs.marltracking.target_track import MARL_TrackEnv
...
def make_train_env(all_args):
    def get_env_fn(rank):
        def init_env():
            if all_args.env_name == "MARL_TRACKING":
                env = MARL_TrackEnv(all_args)
            else:
...
def parse_args(args, parser):
    parser.add_argument('--scenario_name', type=str,
                        default='task1', help="Which task to run on")
...
def main(args):
    ...
    if all_args.share_policy:
        from onpolicy.runner.shared.tracking_runner import TrackingRunner as Runner
    else:
        from onpolicy.runner.separated.mpe_runner import MPERunner as Runner

    runner = Runner(config)
    runner.eval(total_num_steps = 600, for_paper_evaluation = True)
