...
import shutil
import torch
from model import WTFTP, WTFTP_AttnRemoved, WTFTPv2
from torch.utils.data import DataLoader
from dataloader import DataGenerator
from utils import progress_bar, recorder
import argparse
import logging
import datetime
...
class Train:
    def __init__(self, opt, SEED=None, tracking=False):
        ...
        if self.opt.attn:
            ...
            self.net = WTFTP_AttnRemoved(n_inp=3,
                                         n_oup=3,
                                         n_embding=opt.embding,
    ...
    def train_each_epoch(self, epoch):
        ...
        for i, batch in enumerate(train_data):
            ...
            wt_tgt_batch = dwt(batch.transpose(1, 2).contiguous())  # tuple(lo, hi), shape: batch * n_attr * n_sequence
            ...
            pre_batch = idwt((wt_pre_batch[-1].transpose(1, 2).contiguous(),
                              [comp.transpose(1, 2).contiguous() for comp in wt_pre_batch[:-1]])).contiguous()  # shape: batch * n_attr * n_sequence
            ...
            loss_backward = torch.tensor(self.opt.w_spatial, dtype=torch.float, device=self.device) * train_temporal_loss + train_wt_loss
...
if __name__ == '__main__':
    ...
    parser.add_argument('--saving_model_num', default=3, type=int)
