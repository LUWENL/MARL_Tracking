...
parser.add_argument('--minibatch_len', default=5+1, type=int)
parser.add_argument('--pre_len', default=1, type=int)
parser.add_argument('--interval', default=1, type=int)
parser.add_argument('--batch_size', default=1, type=int)
parser.add_argument('--cpu', action='store_true')
parser.add_argument('--logdir', default='./log', type=str)
parser.add_argument('--datadir', default='./transformed_data/', type=str)
# parser.add_argument('--netdir', default='../MusDev7_wtftp_model/pretrained_model/model.pt', type=str)
# parser.add_argument('--netdir', default='../MusDev7_wtftp_model/log/25-10-17/epoch_216.pt', type=str)
parser.add_argument('--netdir', default='../MusDev7_wtftp_model/pretrained_models/net1.pt', type=str)
# parser.add_argument('--netdir', default='../MusDev7_wtftp_model/log/25-10-17/epoch_300_4090.pt', type=str)
...
class Test:
    def __init__(self, opt, net=None):
        ...
        self.data_set = DataGenerator(data_path=self.opt.datadir,
                                      minibatch_len=opt.minibatch_len, interval=opt.interval, train=False,
                                      test_shuffle=True)
        self.net = net
    ...
    def load_model(self, model_path):
        ...
        self.net = torch.load(model_path, map_location=self.device, weights_only=False)
    ...
    def test(self):
        ...
        test_data = DataLoader(dataset=self.data_set.test_set[:100], batch_size=self.opt.batch_size, shuffle=False,
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(test_data):
                ...
                for j in range(self.opt.pre_len):
                    if j > 0:
                        ...
                        inp_batch = torch.cat((inp_batch[:, 1:, :], new_batch),
                                              dim=1)  # shape: batch * his_len * n_attr
                    ...
                    pre_batch = idwt((wt_pre_batch[-1].transpose(1, 2).contiguous(), [comp.transpose(1, 2).contiguous() for comp in wt_pre_batch[:-1]])).contiguous()
                    ...
                    # print(pre_batch[:, self.opt.minibatch_len - self.opt.pre_len, :].shape)

                    # print(inp_batch)
                    # print('\n', pre_batch[:, self.opt.minibatch_len - self.opt.pre_len, :])
            ...
            # print(tgt_set.shape, pre_set.shape)
            ...
            for i in range(self.opt.pre_len):
                ...
                for j, name in enumerate(self.data_set.attr_names):
                    ...
                    print('MAE', self.data_set.unscale(tgt_set[:, :i + 1, j], name), '\n',
                          self.data_set.unscale(pre_set[:, :i + 1, j], name), '\n', '___' * 10)
                    ...
                    mre[name] = float(torch.mean(torch.abs(self.data_set.unscale(tgt_set[:, :i + 1, j], name) -
                                                 ...
                                                 self.data_set.unscale(tgt_set[:, :i + 1, j], name)[
                                                     logit]).cpu()) * 100 if name in 'lonlatalt' \
                        else "N/A"
