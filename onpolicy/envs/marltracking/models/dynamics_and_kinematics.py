...
def integrate_omega(omega, dt, tau, I, max_omega_for_clip=METADATA['max_omega_for_clip'], t=0):
    def dynamics_func(omega, t):
        omega_cross_matrix = get_cross_matrix(omega)
        omega_dot = np.linalg.inv(I) @ (tau - omega_cross_matrix @ I @ omega)
        return omega_dot

    new_omega = runge_kutta(omega, t, dt, dynamics_func)

    # clip the omega
    # new_omega = np.clip(new_omega, -np.pi * 10, np.pi * 10)
